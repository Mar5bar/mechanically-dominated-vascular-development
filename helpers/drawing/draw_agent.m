function [trails, overwriting] = draw_agent(label,I,trails,allowed)
% DRAW_WALKER draws a agent on trails at (i,j).
% We're not going to check any bounds, but we will check for overwrites/collisions.
    ind = I(1) + (I(2)-1)*size(trails,1);
    if nargout == 2
        % Flag up if we're about to overwrite anything made by another agent.
        test_val = trails(ind);
        if nargin < 4
